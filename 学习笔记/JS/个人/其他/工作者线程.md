...
- `skipWaiting()`：强制服务工作者线程跳过等待进入活动状态；需要跟 `Clients.claim()` 一起使用。
...
本节中涉及注册服务工作者线程的例子都使用脚本绝对 `URL`，以避免混淆。下面第一个例子演示通过根目录获取服务脚本对应的默认根作用域：
...
- 确保服务脚本来自相同的源。
- 确保在安全上下文中注册服务工作者线程。
- 确保服务脚本可以被浏览器 `JavaScript` 解释器成功解析而不会抛出任何错误。
