...
​		`<abbr>` 标签指示某个术语的简称或缩写，比如 "`WWW`" 或 "`NATO`"。通过对缩写进行标记，您能够为浏览器、拼写检查和搜索引擎提供有用的信息。
...
​		在同 `<abbr>` 元素一起使用的时候，`title` 属性有特殊的语法含义，它必须包含对一个缩写完全天然可读的扩写或者描述。这些文本通常会作为提示，在鼠标指针悬停到该元素上时显示出来作为一个提示。
