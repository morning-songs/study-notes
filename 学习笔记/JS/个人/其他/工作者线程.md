...
自我终止和外部终止最终都会执行相同的工作者线程终止例程。如下例，其中工作者线程在发送两条消息中间执行了自我终止：
...
注意：由于 `Spectre` 和 `Meltdown` 的漏洞，所有主流浏览器在 2018 年 1 月就禁用了 `SharedArrayBuffer`。从 2019 年开始，有些浏览器开始逐步重新启用这一特性。既不克隆，也不转移，`SharedArrayBuffer` 作为 `ArrayBuffer` 能够在不同浏览器上下文间共享。在把 `SharedArrayBuffer` 传给 `postMessage()` 时，浏览器只会传递原始缓冲区的引用。结果是，两个不同的 `JavaScript` 上下文会分别维护对同一个内存块的引用。每个上下文都可以随意修改这个缓冲区，就跟修改常规 `ArrayBuffer` 一样。来看下面的例子：
