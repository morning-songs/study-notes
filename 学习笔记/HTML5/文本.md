...
​		实际上，我们会较少地使用 `<em>` 标签，至于 `<strong>` 标签出现的次数则更少。如果说用 `<em>` 标签修饰的文本好像是在大声呼喊，那么用 `<strong>` 标签修饰的文本就无异于尖叫了。限制 `<strong>` 的使用会使其文字更加引人注意，而且更加有效。`<strong>` 通常用于摘要，因为它不仅概括了其所在页面的内容，而且位于页面的最重要的位置，其内容自然是非常重要且值得强调的。
